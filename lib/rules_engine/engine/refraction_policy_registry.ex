defmodule RulesEngine.Engine.RefractionPolicyRegistry do
  @moduledoc """
  Registry of built-in refraction policies with discovery and configuration helpers.

  Provides standardised access to refraction policies for host applications.
  """
  ...
  @built_in_policies [
    {:default, DefaultRefractionPolicy},
    {:none, NoRefractionPolicy},
    {:per_rule, PerRuleRefractionPolicy},
    {:ttl, TtlRefractionPolicy}
  ]

  @doc """
  Get all available refraction policies.
  ...
  @doc """
  Resolve refraction policy from atom or module.

  ## Examples

      iex> resolve_policy(:default)
      {:ok, RulesEngine.Engine.DefaultRefractionPolicy}
  ...
  def resolve_policy(policy) when is_atom(policy) do
    case Keyword.get(@built_in_policies, policy) do
      nil when policy in [nil, :default] ->
        {:ok, DefaultRefractionPolicy}

      nil ->
        # Check if it's a module implementing the behaviour
        if implements_refraction_policy?(policy) do
          {:ok, policy}
        else
          {:error, :unknown_policy}
        end
  ...
  ## Examples
      ...
      iex> policy_info(RulesEngine.Engine.DefaultRefractionPolicy)
      {:ok, %{name: "default_per_activation", description: "Prevents rule..."}}
  """
  @spec policy_info(module()) :: {:ok, map()} | {:error, :invalid_policy}
  def policy_info(policy_module) when is_atom(policy_module) do
    if implements_refraction_policy?(policy_module) do
      name =
  ...
  defp implements_refraction_policy?(module) when is_atom(module) do
    case Code.ensure_loaded(module) do
      {:module, ^module} ->
        module.__info__(:attributes)
        |> Keyword.get(:behaviour, [])
        |> Enum.member?(RefractionPolicy)
